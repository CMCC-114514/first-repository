    ...
    static void main() {
        ...
        if (choose == 1) {
            ...
            if (choose == 1) {
                ...
                System.out.print(numOfDays + "天是" + DateTools.day_to_year(numOfDays).showNumOfYears());
            } else if (choose == 2) {
                System.out.print("输入起始日期（如2024 8 19）：");
                Date start = new Date(sc.nextInt(), sc.nextInt(), sc.nextInt());
                System.out.println("输入天数：");
                int numOfDays = sc.nextInt();
                System.out.println(start.showDate() + "的" + numOfDays + "天后是" + DateTools.date_plus_day(start, numOfDays).showDate());
            } else if (choose == 3) {
                System.out.print("输入起始日期（如2024 8 19）：");
                Date start = new Date(sc.nextInt(), sc.nextInt(), sc.nextInt());
                System.out.print("输入结束日期（如2025 11 14）：");
                Date end = new Date(sc.nextInt(), sc.nextInt(), sc.nextInt());
                System.out.println(start.showDate() + "到" + end.showDate() + "有" + DateTools.date_interval(start, end) + "天");
            }
        }
    }
}